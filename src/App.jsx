      ...
      <>
        ...
        <div style={{
          ...
          <div style={{ width: "100%", maxWidth: 340, background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 14, padding: 22 }}>
            <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 2 }}>我的工作台</div>
            <div style={{ fontFamily: FONT_MONO, fontSize: 10.5, color: C.textFaint, marginBottom: 18 }}>AI WORK HUB</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <input className="wh-input" placeholder="密碼" type="password" value={authPasswordInput}
                ...
                onKeyDown={e => { if (e.key === "Enter") submitAuth(); }}
                style={{ ...inputStyle(), fontFamily: FONT_BODY }} />
              {authError && <div style={{ fontSize: 12, color: C.danger }}>{authError}</div>}
              {connectionError && <div style={{ fontSize: 12, color: C.danger }}>{connectionError}</div>}
              <button type="button" className="wh-btn" onClick={submitAuth} disabled={authBusy}
                style={{ ...primaryBtnStyle(), padding: "9px 0", fontSize: 13.5, opacity: authBusy ? 0.6 : 1 }}>
                {authBusy ? "處理中…" : (authMode === "login" ? "登入" : "註冊並登入")}
              </button>
            </div>
            <div style={{ fontSize: 11, color: C.textFaint, marginTop: 12, lineHeight: 1.6 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 12, color: C.textDim }}>
              你是：<b style={{ color: C.text }}>{myName}</b>{isBoss && <span style={{ color: C.accent }}>（老闆）</span>}
            </span>
            <IconBtn onClick={logout} title="登出"><LogOut size={14} /></IconBtn>
            <IconBtn onClick={() => setShowSettings(true)} title="設定（工作同仁 / AI 啟用碼 / 老闆）"><Settings size={15} /></IconBtn>
          </div>
        </div>
        ...
        <div className="wh-dashboard" style={{ marginBottom: 12 }}>
          <div className="wh-sidebar">
            <div className="wh-box" style={{ padding: 10 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: C.textDim, marginBottom: 8 }}>檢視</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <div className="wh-colrow" onClick={() => setViewingAs("__all__")}
                  ...
                  <span style={{ fontSize: 12.5, color: viewingAs === "__all__" ? C.accent : C.text, fontWeight: viewingAs === "__all__" ? 700 : 400 }}>全部（所有人）</span>
                </div>
                <div className="wh-colrow" onClick={() => setViewingAs(null)}
                  ...
                  <span style={{ fontSize: 12.5, color: (viewingAs === null) ? C.accent : C.text, fontWeight: (viewingAs === null) ? 700 : 400 }}>我的工作</span>
                </div>
                <div style={{ height: 1, background: C.borderSoft, margin: "6px 2px" }} />
                {colleagues.length === 0 ? (
                  <div style={{ fontSize: 11, color: C.textFaint, padding: "4px 7px" }}>還沒有同仁</div>
                ) : colleagues.map(c => (
                  <div key={c} className="wh-colrow" onClick={() => setViewingAs(c)}
                    ...
                    <span style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
                      ...
                      <span style={{ fontSize: 12.5, color: viewingAs === c ? C.accent : C.text, fontWeight: viewingAs === c ? 700 : 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c}</span>
                    </span>
                    {colleagueTaskCount(c) > 0 && <span style={{ fontFamily: FONT_MONO, fontSize: 10, color: C.textFaint, flexShrink: 0 }}>{colleagueTaskCount(c)}</span>}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="wh-fadein" style={{ position: "absolute", inset: 0, background: "rgba(6,9,14,0.88)", borderRadius: 18, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, zIndex: 10 }}>
            <div style={{ width: "100%", maxWidth: 420, background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 14, padding: 16, boxShadow: "0 20px 50px rgba(0,0,0,0.5)", maxHeight: "85vh", overflowY: "auto" }} className="wh-scroll">
              ...
              <div style={{ borderTop: `1px solid ${C.borderSoft}`, paddingTop: 14, marginTop: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                  <Users size={13} color={C.textDim} /><span style={{ fontSize: 12, fontWeight: 700, color: C.textDim }}>老闆帳號</span>
                </div>
                <div style={{ fontSize: 11, color: C.textFaint, marginBottom: 8, lineHeight: 1.5 }}>
                  設為老闆的帳號，登入後預設會直接看到「全部（所有人）」的總覽，不用另外點選；一般同仁登入後預設只會看到自己的工作。名字要跟登入用的名字完全一樣。
                </div>
                <div style={{ fontSize: 12, color: C.text, marginBottom: 6 }}>目前老闆：<span style={{ fontFamily: FONT_MONO, color: C.accent }}>{bossName || "（尚未設定）"}</span></div>
                <div style={{ display: "flex", gap: 5 }}>
                  <input className="wh-input" placeholder="老闆的登入名字" value={newBossDraft} onChange={e => setNewBossDraft(e.target.value)}
                    onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); adminSetBoss(); } }} style={{ ...inputStyle(), flex: 1, fontSize: 12 }} />

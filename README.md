...
- 每個人第一次打開網站，用「註冊新帳號」設定自己的名字＋密碼；之後這台裝置、這個瀏覽器會記住登入狀態，不用每次重新輸入
- 換裝置或換瀏覽器登入，就用「登入」，輸入同樣的名字＋密碼即可
- 左邊側欄可以切換「全部（所有人）」「我的工作」，或點某位同仁看他完整的工作清單——這一版拿掉了先前「只有協作才看得到彼此工作」的限制，改成同仁之間互相都看得到
- **老闆帳號**：在「設定」裡指定誰是老闆（填的名字要跟他登入用的名字完全一樣）。老闆登入後預設會直接看到「全部（所有人）」，不用另外點選，也可以直接新增工作並指派給任何同仁
- AI 截圖分析預設鎖住，要在「設定」裡的啟用碼功能開通

---

## 測試完、要正式使用前：如何清空資料

不需要重新部署，直接到 Google Sheets 那份試算表操作即可：

1. 打開你的 Google 試算表
2. 分別點開 **Tasks**、**Projects**、**Colleagues**、**Users**、**Settings** 這幾個分頁
3. 每個分頁裡，選取「第一列（標題列）以下」的所有資料列，刪除（保留標題列不要刪，不然程式會抓不到欄位）
4. 全部清完存檔即可，不用改網址、不用重新部署

清掉 **Users** 分頁代表所有人（包含你自己）都要重新註冊一次帳號；清掉 **Settings** 分頁代表老闆名字跟 AI 啟用碼也要重新設定一次。如果只想清工作資料、保留帳號和設定，只清 **Tasks** 這個分頁就好。
...
- **密碼保護的程度**：密碼在瀏覽器裡先做過 SHA-256 雜湊才送出去，Google Sheets 存的不是明文密碼；但這不是業界標準的密碼儲存做法（沒有加鹽、不是設計給密碼用的雜湊演算法），對「同事之間，擋一下不小心誤按或不熟的人」是夠用的，不建議同仁使用跟公司信箱、銀行帳號一樣的密碼在這裡。

## ⚠️ 這一版又改到了 Code.gs，記得重新部署

這次修正了一個比較嚴重的問題：新增工作時，資料可能寫進錯誤的欄位（例如「分配者」欄位顯示成 manual）。原因是欄位對應方式不夠嚴謹，這次改成永遠依照 Google Sheet 當下實際的欄位順序去寫入，徹底解決這類欄位錯位的問題。

1. 回到 Google Sheet 的 Apps Script 編輯器
2. 把整份 `Code.gs` 內容換成這次新的版本
3. 「部署」>「管理部署作業」> 點編輯（鉛筆圖示）> 版本選「新版本」> 部署
4. 網址不會變，`VITE_SHEETS_URL` 不用改

**這次修正的是「以後新增的資料」，不會回頭修正已經寫錯的舊資料。** 如果妳還在測試階段，建議部署完新版後，直接到 Google Sheet 的 Tasks 分頁把測試資料整批清掉（保留標題列），重新開始測試最準。
